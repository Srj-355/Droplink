...
export function usePeer({ onTransferComplete } = {}) {
  ...
  // ── Sender finalize ────────────────────────────────────────────────────────
  // Stable ref — lets _finalizeSender call _advanceQueue without a circular dep
  ...
  // Keep ref always pointing at the latest version
  ...
  const _finalizeSender = useCallback((fileId, state) => {
    ...
    // Advance queue via stable ref — no circular dependency
    activeFileId.current = null;
    _advanceQueueRef.current?.();
  }, [onTransferComplete]); // _advanceQueueRef is a stable ref — intentionally omitted
  ...
  const joinRoom = useCallback(() => {
    ...
    if (!libsReady) { setPeerError("Libraries still loading."); return; }
    setPeerError("");
    // Do NOT setScreen("room") here — wait until setupConn's "open" event fires
    intentionalLeave.current = false; isHost.current = false;
    targetRoomCode.current = code;
  ...
  const _sendFileInternal = useCallback((file, fileId) => {
    ...
    const sendChunk = async (index) => {
      ...
      // Back-pressure check BEFORE reading — avoid buffering chunks in RAM unnecessarily
      const liveDcPre = dcRef.current;
      if (liveDcPre && liveDcPre.bufferedAmount > HIGH_WATERMARK) {
        try { await waitForDrain(); } catch { return; }
      }
      ...
      try { connRef.current.send(frame); }
      catch (err) { console.error("Send err:", err); st.aborted = true; updateTransfer(fileId, { status: "error" }); return; }
